...
import saucedemo.FileUtiles;
import saucedemo.utiles.logs.LogsManager;
import saucedemo.utiles.logs.timeManager;

import java.nio.file.Path;
import java.util.ArrayList;
import java.util.List;

import static saucedemo.utiles.report.AllureConstant.HISTORY_FOLDER;
import static saucedemo.utiles.report.AllureConstant.RESULT_HISTORY_FOLDER;

public class AllureReportGenerator {
    public static void generateReports(boolean isSingleFile) {
        Path outputFolder = isSingleFile ? saucedemo.utiles.report.AllureConstant.REPORT_PATH : saucedemo.utiles.report.AllureConstant.FULL_REPORT_PATH;
        try {
            String allureExecutable = saucedemo.utiles.report.AllureBinaryManager.getExecutable().toAbsolutePath().toString();

            List<String> command = new ArrayList<>(List.of(
                    "cmd.exe", "/c", allureExecutable, "generate",
                    saucedemo.utiles.report.AllureConstant.RESULT_FOLDER.toAbsolutePath().toString(),
    ...
    public static void openReport() {
        try {
            ...
            ProcessBuilder builder = new ProcessBuilder(
                    ...
                    allureExecutable, "serve", saucedemo.utiles.report.AllureConstant.RESULT_FOLDER.toAbsolutePath().toString()
            );
            builder.start();
        } catch (Exception e) {
...
//    }
    ...
    public static String renameReport(){
        String newFileName= saucedemo.utiles.report.AllureConstant.REPORT_PREFIX+ timeManager.getTimestamp()+ saucedemo.utiles.report.AllureConstant.REPORT_EXTENSION;
        FileUtiles.renameFile(saucedemo.utiles.report.AllureConstant.REPORT_PATH.resolve(saucedemo.utiles.report.AllureConstant.INDEX_HTML).toString(),newFileName);
        return newFileName;
    }
